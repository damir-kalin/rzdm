...
class StarRocksDropTableOperator(BaseHooksOperator):
    """
    ...
    def execute(self, context):
        """
        Удаляет таблицу в StarRocks

        Returns:
            bool: True если таблица удалена, False если удаление не требуется
        """
        ...
        try:
            ...
            starrocks_table_manager.drop_table(self.starrocks_table)
            return True
        except Exception as e:
            self.log.error(f"Ошибка при удалении таблицы {self.starrocks_table}: {e}")
            raise
